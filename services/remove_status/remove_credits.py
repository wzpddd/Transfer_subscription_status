...
def remove_account_credits(env,uid_or_email: str, cookies: None):
    ...
    #先进行积分判断，没有积分的不执行下一步
    get_credits_details_api = get_api("user_payment",env=env)
    params = {
        "key": "creditPoints",
        "value":uid,
        "status":"",
    ...
    #转移积分
    get_remove_credits_api = get_api('transfer_credits', env=env)
    params = {
        "fromUid":uid,
